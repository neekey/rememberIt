...
module.exports = {
     ...
     * 初始化用户数据
     * @param token
     * @param {Array} [types]   ['project', 'label']
     * @returns {*}
     */
    initialUser: function( token, types ){
        ...
        return this.command( token, commands ).then(function( rets ){
            var error = [];
            var finalRet = {};
            rets.forEach(function( ret ){
                if( ret.error ){
                    ...
                    if( ret.type == 'project_add' ){
                        finalRet.projectId = ret.id;
                    }
                    else if( ret.type == 'label_add' ){
                        finalRet.labelId = ret.id;
                    }
