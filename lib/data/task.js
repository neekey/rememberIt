...
module.exports = {
            ...
            return self.findOneByTodoistTaskId( newTask.todoist_task_id ).then(function( task ){
                if( task ){
                    ...
                    return user.createTask({
                        todoist_user_id: String( user.todoist_user_id ),
                        name: newTask.name,
                        origin_task_id: String( newTask.todoist_task_id ),
    ...
    update: function( taskId, updateTask ){
        return this.findOneStrict( taskId ).then(function( task ){
            ...
            if( updateTask.current_task_id ){
                updateTask.current_task_id = String( updateTask.current_task_id );
            }
    ...
    findOneByTodoistTaskId: function( todoistTaskId ){
        return Model.task.findAll( { where: { origin_task_id: String( todoistTaskId ) } }).then(function( tasks ){
            return tasks[0];
        });
    },

    findOneByTodoistCurrentTaskId: function( todoistTaskId ){
        return Model.task.findAll( { where: { current_task_id: String( todoistTaskId ) } }).then(function( tasks ){
            return tasks[0];
        });
    },
